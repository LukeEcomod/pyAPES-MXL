...
for n in range(0, Nsteps):
    state, canopyflx, T, H2O = Moss.run_timestep(dt, forcing, lbc, sub_dt=60.0)
...
t = np.linspace(0, t_final, Nsteps) / 3600.0
...
ax2 = plt.gca().twinx()
ax2.plot(t, np.mean(results['Tmoss'], axis=0), 'r-', label='Tmoss [degC]'); plt.legend()
