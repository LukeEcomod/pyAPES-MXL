...
from canopy.canopy_asl import CanopyModel
from canopy.constants import GAS_CONSTANT, MOLAR_MASS_H2O, MOLAR_MASS_AIR, DEG_TO_KELVIN, SPECIFIC_HEAR_AIR_MASS
...
class Model(object):
    ...
    def __init__(self, forcing, gen_para, canopy_para, mxl_para, mxl_initial, nsim=0):
        ...
        self.results = _create_results(gen_para['outputs'],
                                       self.Nsteps,
                                       self.Nsoil_nodes,
                                       self.Ncanopy_nodes,
                                       self.Nplant_types)
...
#            z_asl = 40.0 + 5.0*k # testing growth
#            U_mxl = self.forcing['U'].iloc[k]
#            T_mxl = self.forcing['Ta'].iloc[k]
#            H2O_mxl = self.forcing['H2O'].iloc[k]
#            CO2_mxl = self.forcing['CO2'].iloc[k]
            ...
            # soil values for testing (don't affect anything)
            Ts = 10.0
            Ws = 0.3
            Psis = 1e-2
            ...
            canopy_flux, canopy_state, ffloor_flux, asl_profs = self.canopy_model.run_timestep(
                dt=self.dt,
                z_asl = z_asl,
...
def _to_kinematic_fluxes(H, LE, Fc, T, P, h2o):
    ...
    wt = H / (rhoa * SPECIFIC_HEAR_AIR_MASS) # K m s-1
...
def _create_results(variables, Nstep, Nsoil_nodes, Ncanopy_nodes, Nplant_types):
    ...
    Returns dictionary with list of arrays
    """
