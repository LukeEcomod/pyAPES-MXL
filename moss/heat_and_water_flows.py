...
def water_heat_flow(t_final, z, initial_state, forcing, parameters, steps=10):
    ...
    while t < t_final:
        ...
        while (err_h > crit_h or err_W > crit_W or err_Wice > crit_Wice or err_T > crit_T):
            ...
            #--- solve vapor phase and return E [kg m-2 s-1] & c_vap[kg m-3]
            #print(iterNo, np.shape(h_iter), np.shape(Kvap))
            ...
            Ew = -1e-3 / dz * E # evaporation / condensation [s-1]
            #Ew = np.zeros(N)
            ...
            if iterNo == 20:
                if dt / 3.0 > 11.0:
                    ...
                    print('(iteration %s) Solution not converging, err_h: %.5f, errW: %.5f, errT: %.5f' 
                          %(iterNo, err_h, err_W, err_T))
                    break
...
def solve_vapor(h, T, Kvap, ubc):
    ...
    Args:
        h - pressure head (m)
        T - temperature (degC)
        Kvap - conductivity (ms-1)
        ubc - upper bc (concentration at surface air)
        
    Returns:
        E - evaporation/condensation rate at each layer [kg m-2 s-1]
