...
class MLM_Moss(object):
    ...
    def __init__(self, z, para, initial_state):
        ...
        self.water_content = WATER_DENSITY / self.bulk_density * self.Wliq
    ...
    def _water_flux_constraints(self):
        """ returns available water and available storage space in kg m-2 """
        available_water = (self.Wliq - self.pF['theta_r'] - EPS) * self.dz * WATER_DENSITY
        available_space = (self.pF['theta_s'] - self.Wliq - EPS) * self.dz * WATER_DENSITY
        
        return available_water, available_space

    def layer_energy_balance(self, dt, forcing, Ts):
        """
        computes heat sink/source term from layer energy balance
        Args:
            self - object
            dt - timestep [s]
            forcing (dict):
        ...
        Ta = forcing['T']
        H2Oa = forcing['H2O']
        gr = forcing['radiative_conductance']
        Pamb = forcing['Pamb']
        
        es = 611.0 * np.exp((17.502 * Ts) / (Ts + 240.97))  # Pa
        es = es / Pamb    
        H2Os = self.rh * es # mol/mol
        
        # boundary-layer conductance [molm-2s-1]
        gh = 0.135 * np.sqrt(U / self.length_scale) * self.LAIz
        gv = 0.147 * np.sqrt(U / self.length_scale) * self.LAIz
        gr = gr * self.LAIz
        
        # evaporation / condensation rate
        E_demand = gv * (H2Os - H2Oa) * MOLAR_MASS_H2O # kgm-2s-1
        available_water, available_space = self._flux_constraints() # kgm-2 in layer
        
        E = np.maximum(np.minimum(E_demand, available_water / dt),
                       available_space / dt) #kgm-2s-1
